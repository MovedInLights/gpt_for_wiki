...
1.1. На кириллице:
На регистрацию заявляется словесное обозначение. Словесный элемент «_» выполнен обычным шрифтом на кириллице и является фантазийным относительно товаров и услуг.

1.2. На латинице:
На регистрацию заявляется словесное обозначение. Словесный элемент «» (транслитерация []) выполнен обычным шрифтом на латинице и является фантазийным относительно товаров и услуг.


2.КОМБИНИРОВАННОЕ ОБОЗНАЧЕНИЕ:

2.1. На кириллице:
На регистрацию заявляется комбинированное обозначение. Словесная часть "_" выполнена на кириллице и является фантазийной относительно товаров и услуг. Изобразительный элемент представлен индивидуальным цветовым и шрифтовым написанием словесной части. Графический элемент представлен ..

2.2. На латинице:
На регистрацию заявляется комбинированное обозначение. Словесный элемент «» (транслитерация []) выполнен на латинице и является фантазийным относительно товаров и услуг. Изобразительный элемент представлен индивидуальным цветовым и шрифтовым написанием словесной части. Графический элемент представлен ..


3.ИЗОБРАЗИТЕЛЬНОЕ ОБОЗНАЧЕНИЕ:
На регистрацию заявляется изобразительное обозначение. Графический элемент представлен … и является фантазийным относительно товаров и услуг.
"""
...
COMPARE_PROMPT = f"""
    ...
    "{RUPTO_TOOLKIT_TEXT}"
    YOU SHOULD STRICTLY FOLLOW THIS GUIDELINE!!!!!
    Provide your answer with a percentage probability calculation. You must answer with the likelihood of successful registration of the client's designation as a trademark. Where 0% percent is an extremely low chance of successful registration, 100% is an extremely high chance. You should format answer that way ONLY in Russian, response should be short (max 2 sentenses). Example: 'Вероятность успешной регистрации товарного знака составлят 70%'
